...
class Refinanciamento{
    
    
    public static function sendRefinanciamentoMovimento($convenio, $movimento){
        $url = "emprestimo/refin/{$convenio->CODIGO_BANKTOOLS}";
        $header = array(
        ...
        $data['valortotaloperacao'] = $movimento->VALOROPERACAO;
        $data['valororiginalrepasse'] = $movimento->VALORREPASSE;
        $data['Ade'] = $movimento->ADE_REFIN;
        ...
        if(isset($response->denied)){
            ...
            if($response->data->code == 2){
                ...
                if(isset($response->data->movimento)){
                    $fields = array(
                        'BANKTOOLS_CONVENIO' => $convenio->CODIGO_BANKTOOLS,
    ...
    public static function getRefinanciamentoMovimento($convenio, $movimento){
        $url = "emprestimo/refin/{$convenio->CODIGO_BANKTOOLS}/{$movimento->BANKTOOLS_CODIGO}";
        $header = array(
            "accept: application/json",
            "authorization: {$GLOBALS['API_TOKEN']}",
            "Content-Type: application/json"
        ...
        if(isset($response->denied)){
            ...
            if($response->data->status == 'Negado'){
                ...
                $acaoPROC = 'PEN'; 
            }elseif($response->data->status == 'Averbado' || $response->data->status == 'Averbado Alerta'){
                $acaoPROC = 'APR';
                $fields = array_merge($fields,
                    array(
                        'VALORPARCELARESERVADA' => $response->data->valorparcelareserva,
                        'PRAZORESERVA' => $response->data->prazoreserva,
                ...
                LOG::setLOG('info', "Convenio: ".$convenio->CODIGO_BANKTOOLS.PHP_EOL."Movimento: ".$movimento->CODIGO.PHP_EOL."Retorno com status: ".$response->data->status, __FILE__, __METHOD__, __LINE__);
                return;
            }
        }
        var_dump($fields);
