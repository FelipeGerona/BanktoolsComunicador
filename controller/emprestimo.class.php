...
class Emprestimo{
    ...
    public static function sendEmprestimoMovimento($convenio, $movimento){
        $url = "emprestimo/novo/{$convenio->CODIGO_BANKTOOLS}";
        $header = array(
        ...
        if($movimento->VALORTOTAL) $data['valortotal'] = $movimento->VALORTOTAL;
        if($movimento->VLR_ENCARGOS) $data['valorencargo'] = $movimento->VLR_ENCARGOS;
        if($movimento->VLR_TARIFAS) $data['valortarifa'] = $movimento->VLR_TARIFAS;
        if($movimento->TAXACLIENTE) $data['taxacliente'] = $movimento->TAXACLIENTE;
    ...
    public static function getEmprestimoMovimento($convenio, $movimento){
        $url = "emprestimo/novo/{$convenio->CODIGO_BANKTOOLS}/{$movimento->BANKTOOLS_CODIGO}";
        $header = array(
            "accept: application/json",
            "authorization: {$GLOBALS['API_TOKEN']}",
            "Content-Type: application/json"
