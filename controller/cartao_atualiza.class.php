...
class Cartao_Atualiza{
    
    
    public static function sendCartaoAtualizaMovimento($convenio, $movimento){
        $url = "cartao/atualiza/{$convenio->CODIGO_BANKTOOLS}";
        $header = array(
        ...
        if($movimento->DTNASCIMENTO) $data['dtnascimento'] = $movimento->DTNASCIMENTO;
    ...
    public static function getCartaoAtualizaMovimento($convenio, $movimento){
        $url = "cartao/atualiza/{$convenio->CODIGO_BANKTOOLS}/{$movimento->BANKTOOLS_CODIGO}";
        $header = array(
            "accept: application/json",
            "authorization: {$GLOBALS['API_TOKEN']}",
            "Content-Type: application/json"
